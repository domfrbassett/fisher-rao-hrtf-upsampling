...
Figure~\ref{fig:metric_relationships} compares AIRM with conventional and Bayesian-observer summaries across the 943 completed rows. Because the rows form a repeated subject--method--condition design, Table~\ref{tab:metric_correlations} reports correlations over the 23 method--retention means and gives subject-cluster bootstrap intervals for row-wise Spearman coefficients. AIRM is strongly rank-associated with LSD and with the Bayesian-observer summaries, but the relationships are not linear or one-to-one: methods with similar LSD or localisation summaries can differ appreciably in Fisher-tensor discrepancy.
...
AIRM is a tensor distance rather than an angular threshold. To express selected tensors in psychophysical units, the \(d'=1\) displacement was projected onto lateral tangent directions at \((0^\circ,0^\circ)\), \((45^\circ,0^\circ)\), and \((90^\circ,0^\circ)\), and onto vertical tangent directions at \((0^\circ,0^\circ)\) and \((0^\circ,45^\circ)\). Table~\ref{tab:local_threshold_plausibility} reports these scalar projections in degrees. They are model-derived thresholds under the Barumerli/Reijniers cue-noise convention, not fitted behavioural MAAs.

\input{tables/evaluation/local_threshold_plausibility_table_ieee.tex}

The dense reference predicts increasing lateral thresholds from frontal to lateral directions: \(1.70^\circ\) at \((0^\circ,0^\circ)\), \(3.08^\circ\) at \((45^\circ,0^\circ)\), and \(6.78^\circ\) at \((90^\circ,0^\circ)\). Its vertical thresholds are \(4.44^\circ\) at \((0^\circ,0^\circ)\) and \(5.71^\circ\) at \((0^\circ,45^\circ)\). This ordering is consistent with broadband spatial-resolution data: Stevenson-Hoare reports horizontal-plane means of approximately \(1.5^\circ\), \(2.7^\circ\), and \(7.2^\circ\) at the same azimuths \cite{StevensonHoare2020}, while Grantham et al. report mean MAAs of \(1.6^\circ\), \(2.8^\circ\), and \(6.5^\circ\) for horizontal, diagonal, and vertical wideband-noise trajectories \cite{Grantham2003}. RANF best preserves the five local thresholds at \(N=19\); at \(N=5\), SUpDEq-Bary-MCA and RANF are closest.
