...
The 2023 Bayesian model of Barumerli et al. \cite{Barumerli2023}, used here as a localisation-model comparator, was calibrated against aggregate lateral, polar, and quadrant errors. A subsequent extension introduced likelihood-based fitting to complete response distributions and validated recovery of a reduced parameter set \cite{Barumerli2026}. Both formulations predict localisation responses rather than the local sensory sensitivity considered here. The model's final response stage applies isotropic von Mises--Fisher scatter around each maximum a posteriori (MAP) estimate. This conditional scatter cannot represent direction-dependent anisotropy in localisation uncertainty; accordingly, the simulated response covariances in Fig.~\ref{fig:kent_map_scatter} are approximately isotropic. The total response covariance additionally reflects variability in the noisy MAP estimates and therefore cannot be interpreted directly as local sensory precision. This distinction is important for HRTF upsampling, since a reconstruction may preserve average spectral content while altering the scale, anisotropy, or orientation of local spatial discriminability. A tensorial analysis of sensory cue gradients provides a complementary means of quantifying these changes alongside localisation-model metrics.
