...
\hypersetup{
  pdftitle={A Fisher Information-based Evaluation of the Preservation of Spatial Discriminability in HRTF Upsampling},
...
\thanks{Dominic Bassett is associated with Birmingham City University, Birmingham, U.K.}}
...
\begin{abstract}
HRTF upsampling is typically evaluated using signal-domain reconstruction metrics and, more recently, statistical localisation models, but neither directly assesses the preservation of local spatial discriminability. We introduce an evaluation method based on Fisher information over the sphere of source directions. Spatial gradients of positive spectral gradient, interaural level difference, and interaural time difference features define a bivariate Fisher information tensor, whose inverse gives the Cramér–Rao lower bound on the covariance of any unbiased local angular estimator. Reference and reconstructed tensors are compared using the affine-invariant Riemannian metric, which captures changes in the scale, anisotropy, and orientation of local spatial discrimination ellipses. We evaluate the method on a 41-subject SONICOM cohort under four sparse-grid retention conditions. We compare SUpDEq-based interpolation, RANF, and FSP-AE, reporting Fisher-tensor error alongside log-spectral distance (LSD) and Bayesian localisation metrics. RANF achieves the lowest LSD across all conditions and the lowest tensor error at the three sparsest grids, while FSP-AE maintains stable tensor errors despite higher LSD. Projected \(\bm{d'=1}\) thresholds express selected tensors in angular units and show the expected increase in lateral threshold from frontal to lateral directions. The proposed evaluation therefore quantifies reconstruction errors in predicted local spatial discriminability that are not captured by signal-fidelity or localisation metrics alone.
\end{abstract}
...
Head-related transfer functions (HRTFs) describe the direction-dependent filtering introduced by a listener’s head, torso, and pinnae. Because dense individual HRTF measurements are costly to obtain, spatial upsampling is used to reconstruct a dense directional field from a sparse set of measured directions. Upsampling methods are usually evaluated using log-spectral distance (LSD), interaural level-difference error, interaural time-difference error, and localisation-model metrics. These measures quantify signal fidelity or predicted localisation performance, but do not directly assess whether the reconstructed field preserves local cue-based discriminability predicted by an auditory observer model. The relevant object of evaluation is therefore not only the HRTF at a given direction, but also the local variation of localisation cues over the sphere.

Spatial discrimination studies often report lateral and polar acuity separately, using minimum audible angles or response variability along azimuthal and elevational dimensions. Starch \cite{Starch1905} combined horizontal and vertical discrimination measurements into ``auditory discrimination ellipses'', recognising that spatial acuity depends on both source direction and displacement direction. The ellipse axes were defined by thresholds measured separately in the horizontal and vertical planes, with intermediate-direction thresholds inferred from the resulting ellipse. The measurements used a 100-Hz electric-fork stimulus and therefore lacked the cross-band spectral structure needed for elevation discrimination.

Modern localisation data further suggest that spatial error cannot generally be treated as independent along azimuthal and elevational dimensions. Carlile et al. \cite{Carlile1997} modelled localisation error distributions using Fisher or Kent distributions. In the pooled data, error distributions at 50 of 76 target locations (66\%) were classified as Kent distributions. Their major axes often departed from the azimuth/elevation axes and, rather than following the cones of confusion expected under orthogonal cue processing, showed co-latitude alignment at lateral and posterior locations and radial alignment near the frontal midline.

HRTF reconstruction is also evaluated using auditory observer models that convert an HRTF set into predicted localisation responses. The Bayesian model of Barumerli et al. \cite{Barumerli2023} combines HRTF-derived sensory evidence with a spatial prior, a maximum a posteriori (MAP) decision rule, and post-decision response variability. It therefore provides predicted point estimates from which lateral, polar, and quadrant errors can be calculated, and is used here to assess the predicted localisation performance of each reconstruction. The original model was calibrated against aggregate localisation errors; a later extension introduced likelihood-based fitting to complete response distributions \cite{Barumerli2026}. Neither formulation directly quantifies local discrimination between neighbouring directions. The model's final response stage applies isotropic von Mises--Fisher scatter around each MAP estimate, so this conditional scatter cannot represent direction-dependent anisotropy in localisation uncertainty. Accordingly, the simulated response covariances in Fig.~\ref{fig:kent_map_scatter} are approximately isotropic. The total response covariance also includes variability in the noisy MAP estimates and therefore cannot be interpreted directly as local sensory precision. This distinction matters for HRTF upsampling because a reconstruction may preserve predicted pointing performance while altering the scale, anisotropy, or orientation of local spatial discriminability. A tensorial analysis of sensory cue gradients provides a complementary means of quantifying these changes.

Fisher information provides such a local tensor, quantifying the sensitivity of expected auditory cues to small changes in source direction. In auditory neuroscience, it has been used to relate binaural cue statistics to neural coding. Harper and McAlpine \cite{Harper2004} analysed the population coding of interaural time differences (ITDs) in the mammalian brainstem and showed that neural tuning distributions can be organised to maximise Fisher information over naturally occurring cue ranges. Pav~ao et al. \cite{Pavao2020} extended this analysis to human listeners, reporting that the spatial distribution of Fisher information derived from HRTF statistics correlates with measured free-field ITD discrimination thresholds.

This paper evaluates HRTF spatial upsampling by comparing Fisher information tensors derived from reference and reconstructed auditory cue fields. Let the sound-source direction be \(\mathbf{r}\in\mathbb{S}^2\), and let \(\boldsymbol{\xi}\in\mathbb{R}^{2}\) denote local angular coordinates in an orthonormal tangent basis at \(\mathbf{r}\). Within the assumed cue-noise observer, the Fisher information matrix quantifies local stimulus discriminability on these coordinates. For any unbiased local estimator \(\hat{\boldsymbol{\xi}}\), provided that the Fisher information matrix is nonsingular, the Cram\'er--Rao bound (CRB) states
\begin{equation}
...
\end{equation}
The main evaluation uses \(N_{\max}=6\) for both fields. Using the same SH order makes their gradients comparable at the same spatial resolution.
...
\end{equation}
The implementation uses AMT \path{barumerli2023} with DTF feature extraction, maximum a posteriori estimation, and \(R=300\) simulated repetitions per target direction. Its parameters are \(\sigma_{\mathrm{ITD}}=0.569\), \(\sigma_{\mathrm{ILD}}=1\), \(\sigma_{\mathrm{spectral}}=4\), \(\sigma_{\mathrm{prior}}=11.5\), and \(\sigma_{\mathrm{motor}}=14\). The DTF representation and spectral-noise value follow the public HRTFformer localisation evaluation \cite{Hu2026HRTFformer}; the other parameters follow the AMT 1.6 \path{barumerli2023} defaults. The measured reference template is constructed using the corresponding legacy AMT interpolation, which omits directions below the lowest measured elevation; the later full-sphere extension addresses this limitation \cite{Barumerli2026}. The same reference template is held fixed for the dense self-reference and every reconstruction. The resulting relative metrics therefore compare all conditions under a common observer template.
...
Figure~\ref{fig:bayesian_subject_distributions} reports subject-wise distributions for lateral RMS, local polar RMS, and quadrant error.
...
R. Barumerli, F. Brinkmann, E. Zanoni, A. Hoyer, L. Picinali, and M. Geronazzo, ``Statistical validation and full-sphere extension of a Bayesian model for human static sound localisation,'' arXiv:2606.24367, 2026, doi:10.48550/arXiv.2606.24367.
...
\begin{center}
\includegraphics[width=\columnwidth]{figures/IMG_20250221_124059_862-EDIT.jpg}\\[-0.2em]
