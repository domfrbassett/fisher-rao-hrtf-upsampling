function import_fsp_ae_raw_sofas_for_behavioural_fields(projectRoot, rawSofaRoot, subjectIds, retentions)
%IMPORT_FSP_AE_RAW_SOFAS_FOR_BEHAVIOURAL_FIELDS Convert raw FSP-AE SOFAs to field mats.

    arguments
        projectRoot (1, 1) string = string(fileparts(fileparts(fileparts(mfilename("fullpath")))))
        rawSofaRoot (1, 1) string = fullfile(projectRoot, ...
            "ml_comparator_research", "comparator_protocol", "work", ...
            "ml_lap_aligned", "FSP_AE_raw")
        subjectIds (1, :) double = [100, 80, 33, 104]
    ...
    for iRetention = 1:numel(retentions)
        ...
        sourceFolder = fullfile(rawSofaRoot, sprintf("N%03d", retention));
        assert(isfolder(sourceFolder), ...
            "Raw FSP-AE SOFA folder does not exist: %s", sourceFolder);

        for iSubject = 1:numel(subjectIds)
            subjectId = subjectIds(iSubject);
            sofaPath = fullfile(sourceFolder, sprintf("Sonicom_%d.sofa", subjectId));
            assert(isfile(sofaPath), "Missing raw FSP-AE SOFA: %s", sofaPath);

            field = read_sofa_field_for_behavioural_export(sofaPath);
            field.method = "FSP_AE_raw";
            field.retainedDirections = retention;
            field.sourceSofaPath = string(sofaPath);
            field.provenance = "FSP-AE raw inference SOFA without retained-node replacement";
            ...
            outPath = fullfile(subjectFolder, ...
                sprintf("FSP_AE_raw_N%03d_hrir_field.mat", retention));
